...
def compute_band_powers(eegdata, fs):
    ...
    Y = np.fft.fft(dataWinCenteredHam, n=NFFT, axis=0) / winSampleLength    # contains the complex numbers from FFT
    PSD = 2 * np.abs(Y[0:int(NFFT / 2), :])         # takes magnitude of the cplx numbers (contains the eeg value frequency bins)

    # MODIFIED:
    output = np.concatenate(PSD, axis=0)
    #output = np.log10(output)
    return output

    # # UNCOMMENTED:
    # f = fs / 2 * np.linspace(0, 1, int(NFFT / 2))   # fixed array values
    # # SPECTRAL FEATURES
